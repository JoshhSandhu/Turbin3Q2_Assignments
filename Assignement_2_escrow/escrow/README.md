...
The escrow PDA is derived from `["escrow", maker, seed]` and stores everything `take` and `refund` need to validate themselves:
...
`take` carries the most accounts of the three, so the mints and token accounts are `Box`ed. Without that, `try_accounts` overflows the 4KB stack frame.
...
The same run with `--nocapture`, showing the transactions actually landing and what they cost. `take` is the expensive one at ~98k CUs, since it creates two ATAs, does two transfers and closes an account:
...
`test_make_and_take` and `test_take_after_expiry` share a `setup_escrow` helper that builds two mints, funds a maker and a taker, and runs `make`.
